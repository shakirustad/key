...
def add_to_cart(request, uid):
    ...
    # Get size_variant only if a size was provided
    size_variant = None
    if size:
    ...
    # Check if item exists in cart (considering size if applicable)
    ...
    messages.success(request, f'Added {quantity} item(s) to cart!')
...
def cart(request):
    ...
    context = {'cart': cart_obj, 'payment': payment, 'quantity_range': range(1, 10), }
